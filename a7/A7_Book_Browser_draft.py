...
class BookBrowser:
    ...
    def display_rows(self, rows):
        ...
        try:
            ...
            r = 2
            for book in rows:
                tkinter.Radiobutton(self.main_window, text=book[1], variable=self.book_isbn, value=book[0], command=self.get_book_details, padx=padding_side, font=font, background=bg_color).grid(row=r, column=0)
                tkinter.Label(self.main_window, text=book[2], padx=padding_side, font=font, background=bg_color).grid(row=r, column=1)
                r += 1
        except IndexError as err:
            print('Index error: ', err)
        except Exception as err:
